...
export default function Home() {
  ...
  const handleParticipantsChange = useCallback((newParticipants: AIAgent[]) => {
    setParticipants(newParticipants);
    if (typeof window !== 'undefined') {
      localStorage.setItem('debate-participants', JSON.stringify(newParticipants));
    }
  }, []);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        if (typeof window !== 'undefined') {
          const savedParticipants = localStorage.getItem('debate-participants');
